...
class DTLSReliableHandshake {
  ...
  static DTLSRequest readClientRequest(
      byte[] data, int dataOff, int dataLen, OutputStream dtlsOutput) throws IOException {
    ...
    ClientHello clientHello =
        ClientHello.parse(
            new ByteArrayInputStream(message, MESSAGE_HEADER_LENGTH, length), dtlsOutput);

    return new DTLSRequest(recordSeq, message, clientHello);
  }

  static void sendHelloVerifyRequest(DatagramSender sender, long recordSeq, byte[] cookie)
      throws IOException {
  ...
  DTLSReliableHandshake(
      TlsContext context, DTLSRecordLayer transport, int timeoutMillis, DTLSRequest request) {
    ...
    if (null != request) {
      ...
      DTLSReassembler reassembler =
          new DTLSReassembler(HandshakeType.client_hello, message.length - MESSAGE_HEADER_LENGTH);
  ...
  Message receiveMessage() throws IOException {
    ...
    for (; ; ) {
      ...
      System.out.println("==> receiveLimit: " + receiveLimit);
  ...
  private Message getPendingMessage() throws IOException {
    DTLSReassembler next =
        (DTLSReassembler) currentInboundFlight.get(Integers.valueOf(next_receive_seq));
    if (next != null) {
      byte[] body = next.getBodyIfComplete();
      if (body != null) {
        previousInboundFlight = null;
        return updateHandshakeMessagesDigest(
            new Message(next_receive_seq++, next.getMsgType(), body));
      }
    }
    return null;
  }
  ...
  private void processRecord(int windowSize, int epoch, byte[] buf, int off, int len)
    ...
    while (len >= MESSAGE_HEADER_LENGTH) {
      ...
      if (message_seq >= (next_receive_seq + windowSize)) {
        ...
        DTLSReassembler reassembler =
            (DTLSReassembler) currentInboundFlight.get(Integers.valueOf(message_seq));
        if (reassembler == null) {
          reassembler = new DTLSReassembler(msg_type, length);
          currentInboundFlight.put(Integers.valueOf(message_seq), reassembler);
        }

        reassembler.contributeFragment(
            msg_type, length, buf, off + MESSAGE_HEADER_LENGTH, fragment_offset, fragment_length);
        ...
        DTLSReassembler reassembler =
            (DTLSReassembler) previousInboundFlight.get(Integers.valueOf(message_seq));
        if (reassembler != null) {
          reassembler.contributeFragment(
              msg_type, length, buf, off + MESSAGE_HEADER_LENGTH, fragment_offset, fragment_length);
