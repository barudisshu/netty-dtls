...
class DTLSRecordLayer implements DatagramTransport {
  ...
  static byte[] receiveClientHelloRecord(byte[] data, int dataOff, int dataLen) throws IOException {
    ...
    return TlsUtils.copyOfRangeExact(
        data, dataOff + RECORD_HEADER_LENGTH, dataOff + RECORD_HEADER_LENGTH + length);
  }

  static void sendHelloVerifyRequestRecord(DatagramSender sender, long recordSeq, byte[] message)
      throws IOException {
  ...
  private HeartbeatMessage heartbeatInFlight =
      null; // The current in-flight heartbeat request, if any
  private Timeout heartbeatTimeout =
      null; // Idle timeout (if none in-flight), else expiry timeout for response

  private int heartbeatResendMillis =
      -1; // Delay before retransmit of current in-flight heartbeat request
  private Timeout heartbeatResendTimeout =
      null; // Timeout for next retransmit of the in-flight heartbeat request
  ...
  public int getReceiveLimit() throws IOException {
    return Math.min(
        ...
        readEpoch
            .getCipher()
            .getPlaintextLimit(transport.getReceiveLimit() - RECORD_HEADER_LENGTH));
  }
  ...
  public int receive(byte[] buf, int off, int len, int waitMillis) throws IOException {
    ...
    while (waitMillis >= 0) {
      ...
      System.out.println("==> " + heartbeatTimeout + ": " + currentTimeMillis);
      ...
      if (Timeout.hasExpired(heartbeatTimeout, currentTimeMillis)) {
        ...
        this.heartbeatInFlight =
            HeartbeatMessage.create(
                context, HeartbeatMessageType.heartbeat_request, heartbeat.generatePayload());
      ...
      waitMillis =
          Timeout.constrainWaitMillis(waitMillis, heartbeatResendTimeout, currentTimeMillis);
  ...
  private int processRecord(int received, byte[] record, byte[] buf, int off) throws IOException {
    ...
    } else if (recordType == ContentType.handshake
        && null != retransmitEpoch
    ...
    TlsDecodeResult decoded =
        recordEpoch
            .getCipher()
            .decodeCiphertext(
                macSeqNo, recordType, recordVersion, record, RECORD_HEADER_LENGTH, length);
      ...
      boolean isHelloVerifyRequest =
          getReadEpoch() == 0
              ...
              && HandshakeType.hello_verify_request
                  == TlsUtils.readUint8(record, RECORD_HEADER_LENGTH);
          ...
          if (null != heartbeatInFlight || heartBeatResponder) {
            try {
              ByteArrayInputStream input =
                  new ByteArrayInputStream(decoded.buf, decoded.off, decoded.len);
                  ...
                  case HeartbeatMessageType.heartbeat_response:
                    {
                      if (null != heartbeatInFlight
                          && Arrays.areEqual(
                              heartbeatMessage.getPayload(), heartbeatInFlight.getPayload())) {
                        resetHeartbeat();
                      }
                      break;
                    }
